            ...
            onPress={() =>
              navigation.dispatch(
                NavigationActions.reset({
                  index: 0,
                  actions: [
                    NavigationActions.navigate({ routeName: "Map", key: "Map" })
                  ]
                })
              )
            }
...
export class App extends Component<IProps, IState> {
  ...
  constructor(props: IProps) {
    ...
    this.processDeliveredNotifications = this.processDeliveredNotifications.bind(
      this
    );
  ...
  componentDidMount() {
    ...
    PushNotificationIOS.getDeliveredNotifications(
      this.processDeliveredNotifications
    );
  ...
  async processNotification(
    initialNotification: boolean = true,
  ...
  async setupLocationTracking() {
    ...
    if (this.monitoringLocation) {
      Logger.trace(
        "App.setupLocationTracking - we already set up location tracking, returning"
      );
      return;
    }
    ...
    let postUrl =
      this.configGlobal.apiServicesUrlBase +
      this.configGlobal.apiStage +
      "/postNode";
    ...
    BackgroundGeolocation.ready(
      ...
      state => {
        Logger.info(
          `BackgroundGeolocation is configured and ready: ${state.enabled}`
        );
  ...
  async onLocation(location) {
    ...
    if (!this.nodeService.monitoring) {
      Logger.info(
        "App.onLocation - got first location, starting to monitor nodes"
      );
  ...
  async getPostParams() {
    ...
    try {
      ...
      Logger.debug(
        `App.getPostParams - error getting UUID: ${JSON.stringify(err)}`
      );
    }
    ...
    try {
      ...
      Logger.trace(
        `App.getPostParams - User has a pushy token of: ${deviceToken}`
      );
    } catch (err) {
...
function mapDispatchToProps(dispatch: Dispatch<IStoreState>) {
  return {
    ...
    TrackedNodeListUpdated: bindActionCreators(
      TrackedNodeListUpdatedActionCreator,
      dispatch
    ),
    ...
    RelationListUpdated: bindActionCreators(
      RelationListUpdatedActionCreator,
      dispatch
    ),
    ...
    TransactionListUpdated: bindActionCreators(
      TransactionListUpdatedActionCreator,
      dispatch
    ),
