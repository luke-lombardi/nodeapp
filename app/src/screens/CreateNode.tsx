...
export class CreateNode extends Component<IProps, IState> {
          ...
          <View style={styles.switchView}>
            ...
            <View style={styles.sliderContainer}>
              <Text style={styles.sliderTextContainer}>
                ...
                >
                  {this.state.ttl.toFixed(0)}{" "}
                  {this.state.ttl < 1.1 ? "hour" : "hours"}
                </Text>
              </Text>
              <Slider
                style={[styles.slider, { width: WINDOW_WIDTH * 0.9 }]}
  ...
  private checkForOverlaps() {
    ...
    if (this.props.privatePlaceList.length > 0) {
      if (
        this.props.privatePlaceList[0].data.distance_in_meters <=
        this.configGlobal.minimumNodeDistance
      ) {
        overlaps = true;
        return overlaps;
      }
    ...
    if (this.props.publicPlaceList.length > 0) {
      if (
        this.props.publicPlaceList[0].data.distance_in_meters <=
        this.configGlobal.minimumNodeDistance
      ) {
        overlaps = true;
        return overlaps;
  ...
  private async submitCreateNode() {
    ...
    if (newUuid !== undefined) {
      ...
      Logger.info(
        "CreateNode.submitCreateNode - successfully created new private node."
      );
    } else if (newUuid !== undefined && nodeData.private === false) {
      Logger.info(
        "CreateNode.submitCreateNode - successfully created new public node."
      );
    } else {
      Logger.info(
        "CreateNode.submitCreateNode - invalid response from create node."
      );
    }
