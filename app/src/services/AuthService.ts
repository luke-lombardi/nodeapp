...
export default class AuthService {
  ...
  public static async checkPermissions(showSnackbar: boolean) {
    if (!(await AuthService.hasPermissions())) {
      Logger.info(
        `AuthService.checkPermissions() - user does not have required permissions`
      );
      if (showSnackbar) {
        Snackbar.show({
  ...
  public static async permissionsGranted() {
    ...
    Logger.info(
      `AuthService.permissionsGranted() - Current permissions ${JSON.stringify(
        permissions
      )} `
    );
  ...
  public static async storeWallet(privateKey: string) {
    ...
    Logger.trace(
      `AuthService.storeWallet() - current private key: ${walletPrivateKey}`
    );
  ...
  public static async storeTransaction(txHash: string) {
    ...
    if (index < 0) {
      ...
      Logger.info(
        `AuthService.storeTransaction: now tracking ${trackedTransactions}`
      );

      await AsyncStorage.setItem(
        "trackedTransactions",
        JSON.stringify(trackedTransactions)
      );

      return true;
    } else {
      Logger.trace(
        `AuthService.storeTransaction: you already are tracking this tx.`
      );
      return false;
    }
  }
